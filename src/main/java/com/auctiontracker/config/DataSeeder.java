...
public class DataSeeder implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        player("Arjun Sharma", BATSMAN, A,             142, 4820, 41.2, 138.5, null, null);
        player("Marcus Bell", BATSMAN, A,              165, 5602, 43.8, 142.1, null, null);
        player("Dale Rivers", BOWLER, A,               130,  312, 12.4, 110.0,  168,  7.4);
        player("Rohan Iyer", ALL_ROUNDER, A,           118, 2140, 28.9, 131.0,   96,  8.1);
        // Group B
        player("Kane Foster", WICKETKEEPER, B,         121, 3230, 34.0, 128.7, null, null);
        player("Vikram Rao", BOWLER, B,                 98,  145,  9.1,  92.0,  121,  7.9);
        player("Siddharth Jain", BATSMAN, B,           104, 2980, 36.3, 125.4, null, null);
        player("Liam Carter", ALL_ROUNDER, B,           89, 1675, 26.2, 134.9,   74,  8.4);
        player("Aditya Kulkarni", WICKETKEEPER, B,      76, 1890, 31.5, 122.0, null, null);
        // Group C
        player("Nikhil Verma", BOWLER, C,               67,   88,  8.0,  85.0,   78,  8.2);
        player("Dev Patel", BATSMAN, C,                 58, 1450, 29.0, 119.8, null, null);
        player("Owen Blake", BOWLER, C,                 72,   60,  6.7,  79.0,   84,  7.7);
        player("Karan Mehta", ALL_ROUNDER, C,           63,  940, 22.4, 126.3,   41,  8.6);
        player("Ishaan Reddy", WICKETKEEPER, C,         49, 1020, 26.8, 117.5, null, null);
        // Group D
        player("Sam Turner", BATSMAN, D,                34,  720, 24.0, 121.2, null, null);
        player("Rahul Nair", BOWLER, D,                 28,   35,  7.0,  74.0,   31,  8.0);
        player("Pranav Joshi", BATSMAN, D,              22,  410, 20.5, 113.6, null, null);
        // Group E (lowest — unsold returns to the pool, never terminal)
        player("Harsh Gupta", ALL_ROUNDER, E,           26,  305, 17.9, 118.0,   19,  8.8);
        player("Tejas Kadam", BOWLER, E,                19,   22,  5.5,  68.0,   24,  7.6);
        player("Manoj Pillai", WICKETKEEPER, E,         17,  265, 18.9, 109.1, null, null);
    ...
    private void team(String name, String owner) {
        ...
        // No role minimums — display-only details.
        core.registerTeam(name, owner, d.startingPurse(), d.maxSquadSize(), Map.of());
    }

    private void player(String name, PlayerRole role, PlayerCategory category,
